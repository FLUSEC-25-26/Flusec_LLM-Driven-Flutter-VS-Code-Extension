...
export async function runAnalyzer(doc: vscode.TextDocument, context: vscode.ExtensionContext): Promise<void> {
    ...
    // 🛡️ Robust JSON validation & extraction (Ignores debug text)
    let findings: any[] = [];
    try {
        // Extract JSON array ignoring surrounding debug text
        ...
        if (jsonMatch) {
            ...
            if (singleMatch) {
                ...
                vscode.window.showErrorMessage(`FLUSEC Analyzer Error: No valid JSON found. Check debug console.`);
                return;
            }
        }
        ...
        vscode.window.showErrorMessage("FLUSEC: Failed to parse analyzer output.");
        return;
    }
