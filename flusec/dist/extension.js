...
async function runAnalyzer(context, analyzerArgs, cwd) {
  ...
  if (!cmd) {
    vscode2.window.showErrorMessage("FLUSEC: analyzer not found. Put dart_analyzer(.exe) in flusec/bin/ or dart-analyzer/bin/.");
    return;
  }
  ...
  for (const f of findings) {
    ...
    if (hasLineCols) {
      range = new vscode2.Range(
        new vscode2.Position(startLC.line - 1, startLC.column - 1),
        new vscode2.Position(endLC.line - 1, endLC.column - 1)
      );
    } else {
