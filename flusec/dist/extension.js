...
function extractEndpointsFromDart(doc) {
  ...
  for (const rx of patterns) {
    for (const m of text.matchAll(rx)) {
      ...
      if (url) out2.push({ url, line });
    }
  }
  return out2;
}
function autoServiceId(urlStr) {
  try {
...
function resolveServiceIdFlexible(url, cfg) {
  if (cfg) {
    try {
      const u = new URL(url);
      const host = u.hostname.toLowerCase();
...
function buildCouplingFromCalls(calls, cfg) {
  ...
  for (const c of calls) {
    ...
    modules.add(from);
    services.add(toSvc);
    (edgesOut[from] ||= /* @__PURE__ */ new Set()).add(toSvc);
    (edgesIn[toSvc] ||= /* @__PURE__ */ new Set()).add(from);
  }
  const cdOut = {};
  const afferent = {};
...
function computeHealthIndex(payload, opts) {
  ...
  if (declared.length && payload.services?.length) {
    ...
    redundancyRatio = clamp100(count / payload.services.length * 100) / 100;
  }
...
async function computeAndPostCoupling(panel) {
  ...
  const g = buildCouplingFromCalls(calls, cfg || void 0);
...
function getNetChartMessagingScript(nonce) {
  return `
<script nonce="${nonce}">
...
Redundancy: \${Math.round(((c.redundancyRatio ?? 0) * 100))}%\`;
  ...
  function filterList(q) {
    ...
    // \u{1F504} Update warnings chart with filtered set
...
// src/extension.ts
var collection = vscode2.languages.createDiagnosticCollection("flusec");
var out = vscode2.window.createOutputChannel("FLUSEC");
var tempPathToUntitledUri = /* @__PURE__ */ new Map();
...
function activate(context) {
  ...
  const openDashboardCmd = vscode2.commands.registerCommand("flusec.openDashboard", async () => {
    const panel = vscode2.window.createWebviewPanel(
      ...
      vscode2.ViewColumn.One,
      {
        ...
        localResourceRoots: [
          vscode2.Uri.file(path.join(context.extensionPath, "src", "web"))
        ]
      }
    );
    ...
    const styleHref = panel.webview.asWebviewUri(
      vscode2.Uri.file(path.join(context.extensionPath, "src", "web", "style.css"))
    ).toString();
    const chartHref = panel.webview.asWebviewUri(
      vscode2.Uri.file(path.join(context.extensionPath, "src", "web", "chart.min.js"))
    ).toString();
    ...
    panel.webview.onDidReceiveMessage(async (msg) => {
      switch (msg?.type) {
        ...
        case "openFile": {
          const fsPath = msg?.payload;
          if (fsPath) {
            const doc = await vscode2.workspace.openTextDocument(vscode2.Uri.file(fsPath));
  ...
  const hoverProvider = vscode2.languages.registerHoverProvider({ language: "dart" }, {
...
`);
  ...
  context.subscriptions.push(vscode2.workspace.onDidOpenTextDocument((doc) => {
    if (doc.languageId === "dart") scanDocument(context, doc);
  }));
  context.subscriptions.push(vscode2.workspace.onDidSaveTextDocument((doc) => {
    if (doc.languageId === "dart") scanDocument(context, doc);
  }));
  context.subscriptions.push(vscode2.window.onDidChangeActiveTextEditor((ed) => {
    const doc = ed?.document;
    if (doc?.languageId === "dart") scanDocument(context, doc);
  }));
  let debounce;
  context.subscriptions.push(vscode2.workspace.onDidChangeTextDocument((e) => {
    if (e.document.languageId !== "dart") return;
    if (debounce) clearTimeout(debounce);
    debounce = setTimeout(() => scanDocument(context, e.document), 250);
  }));
  const watcher = vscode2.workspace.createFileSystemWatcher("**/*.dart");
...
async function scanAll(context) {
  for (const doc of vscode2.workspace.textDocuments.filter((d) => d.languageId === "dart")) {
    await scanDocument(context, doc);
  }
}
async function scanDocument(context, doc) {
  ...
  try {
    ...
    vscode2.window.showErrorMessage(`FLUSEC: failed to analyze untitled doc (${String(err)})`);
  } finally {
    setTimeout(() => {
      try {
...
function runAnalyzerForPath(context, fsPath) {
...
async function runAnalyzer(context, analyzerArgs, cwd) {
  const cmd = resolveAnalyzerCommand(context);
  if (!cmd) {
    vscode2.window.showErrorMessage("FLUSEC: analyzer not found. Put analyzer(.exe/.dart) in flusec/bin/ or dart-analyzer/bin/.");
    return;
  }
  ...
  const child = (0, import_child_process.spawn)(cmd.cmd, [...cmd.args, ...analyzerArgs], { cwd, shell: false });
  let stdout = "";
  let stderr = "";
  child.stdout.on("data", (d) => stdout += d.toString());
  child.stderr.on("data", (d) => stderr += d.toString());
  child.on("error", (err) => {
    vscode2.window.showErrorMessage(`FLUSEC: analyzer failed to start (${String(err)})`);
...
Raw stdout:
${stdout}`);
      vscode2.window.showErrorMessage("FLUSEC: analyzer output could not be parsed.");
      return;
    }
    ...
    if (Array.isArray(parsed)) {
      ...
      if (srcPath && srcPath.endsWith(".dart")) {
        vscode2.window.showInformationMessage(`FLUSEC: ${parsed.length} findings in ${path.basename(srcPath)}.`);
      }
    } else {
      const findings = parsed.findings ?? parsed.issues ?? [];
      applyDiagnosticsFromFindings(findings);
      if (srcPath && srcPath.endsWith(".dart")) {
        vscode2.window.showInformationMessage(`FLUSEC: ${findings.length} findings in ${path.basename(srcPath)}.`);
      }
    }
  });
}
function applyDiagnosticsFromFindings(findings) {
  const byUriStr = /* @__PURE__ */ new Map();
  for (const f of findings) {
    const analyzedPath = f.file;
    const untitledUri = tempPathToUntitledUri.get(analyzedPath);
    const targetUri = untitledUri ?? vscode2.Uri.file(analyzedPath);
    const startLC = f.range?.start;
    const endLC = f.range?.end;
    let range;
    const hasLineCols = startLC?.line !== void 0 && startLC?.column !== void 0 && endLC?.line !== void 0 && endLC?.column !== void 0;
    if (hasLineCols) {
      range = new vscode2.Range(
        new vscode2.Position(Math.max(0, startLC.line - 1), Math.max(0, startLC.column - 1)),
        new vscode2.Position(Math.max(0, endLC.line - 1), Math.max(0, endLC.column - 1))
      );
    } else {
    ...
    const diag = new vscode2.Diagnostic(range, f.message, vscode2.DiagnosticSeverity.Warning);
  ...
  for (const [uriStr, diags] of byUriStr.entries()) {
    const uri = vscode2.Uri.parse(uriStr);
    collection.set(uri, diags);
    currentDiagnostics.set(uri.fsPath, diags);
  }
}
function applyDiagnosticsFromIssues(issues, srcPath) {
  const targetUri = tempPathToUntitledUri.get(srcPath) ?? vscode2.Uri.file(srcPath);
  const diags = [];
  for (const i of issues) {
    const line = Number(i.line ?? 1);
    const column = Number(i.column ?? 1);
    const start = new vscode2.Position(Math.max(0, line - 1), Math.max(0, column - 1));
    const end = new vscode2.Position(Math.max(0, line - 1), Math.max(0, column));
    const range = new vscode2.Range(start, end);
    const msg = String(i.message ?? i.ruleId ?? "FLUSEC issue");
    const diag = new vscode2.Diagnostic(range, msg, vscode2.DiagnosticSeverity.Warning);
...
function docPosByOffsetForUri(uri, offset) {
  const doc = vscode2.workspace.textDocuments.find((d) => d.uri.toString() === uri.toString());
  if (!doc) return new vscode2.Position(0, 0);
  const text = doc.getText();
  let line = 0, ch = 0;
  ...
  return new vscode2.Position(line, ch);
}
function smokeTestDiagnostics(doc) {
...
function buildDashboardHtml(webview, rawHtml, uris) {
